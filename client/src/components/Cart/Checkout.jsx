...
const Checkout = () => {
  ...
  const handleFinalizeCheckout = async (checkoutId) => {
    try {
      const responce = await axios.post(
        ...
        }/api/checkout/${checkoutId}/finalize`,{},{headers: {
          Authorization: `Bearer ${localStorage.getItem("userToken")}`,
        },
      }
      );
  ...
  if (!cart || !cart.products || cart.products.length === 0) {
    return <p>Your cart is emphty</p>;
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto py-10 px-6 tracking-tight">
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <div className="flex justify-between items-center text-lg mb-4 ">
          <p>Subtotal</p>
          <p>${cart.subtotal?.toLocaleString()}</p> {/* ✅ use subtotal */}
        </div>
        ...
        <div className="flex justify-between text-lg mt-4 border-t pt-4">
          <p>Total</p>
          {/* 🔴 was cart.toLocalprice?.toLocalString() */}
          <p>${cart.totalPrice?.toLocaleString()}</p> {/* ✅ use totalPrice */}
        </div>
      </div>
    </div>
  );
