...
router.get("/", async (req, res) => {
  try {
    ...
    // Collection filter (case-insensitive)
    if (collection && collection.toLowerCase() !== "all") {
      query.collection = { $regex: `^${collection.trim()}$`, $options: "i" };
    }
    ...
    // Material filter (support multiple comma-separated materials, case-insensitive)
    if (material) {
      query.material = {
    ...
    // Brand filter (support multiple comma-separated brands)
    if (brand) {
      query.brand = { $in: brand.split(",").map((b) => b.trim()) };
    }

    // Size filter (support multiple comma-separated sizes)
    if (size) {
      query.sizes = { $in: size.split(",").map((s) => s.trim()) };
    }

    // Color filter (support multiple comma-separated colors)
    if (color) {
...
if (gender) {
  query.gender = { $regex: `^${gender.trim()}$`, $options: "i" };
    ...
    // Search keyword filter (name or description)
    if (search) {
      query.$or = [
...
router.get("/best-seller", async (req, res) => {
  try {
    // Find one product with the highest rating
...
router.get("/similar/:id", async (req, res) => {
  ...
  try {
    ...
    // Find similar products (same gender & category), exclude current product
